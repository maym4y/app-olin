...
export default function NewEvidenceModal({ visible, onClose, casoId, onEvidenceAdded }) {
  ...
  const selecionarArquivo = () => {
    Alert.alert('Selecionar Arquivo', 'Escolha o tipo de evidência que deseja adicionar:', [
      {
        text: 'Cancelar',
        style: 'cancel'
      },
    ...
    try {
      ...
      const arquivoUpload = {
        ...
        type:
          ...
          (formData.tipoArquivo === 'imagem' ? 'image/jpeg' : 'application/pdf'),
        name:
          formData.arquivo.name ||
          `evidencia_${Date.now()}.${formData.tipoArquivo === 'imagem' ? 'jpg' : 'pdf'}`
      };
